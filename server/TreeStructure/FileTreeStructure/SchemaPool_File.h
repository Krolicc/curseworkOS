#ifndef SCHEMAPOOL_FILE_H
#define SCHEMAPOOL_FILE_H

#include <utility>

#include "../../ExtraFunctions.h"

#include "CollectionsSchema_File.h"
...
class SchemaPool_File final : public BTree_File<std::string, std::string, CollectionSchema_File>  {
public:
  std::string rootFile;
  std::string source;

  explicit SchemaPool_File(std::string _source) : source(std::move(_source)) {
  }
  ...
  std::shared_ptr<CollectionSchema_File> load(const std::string &key);
...
class SchemaPoolNode_File final : public BTreeNode_File<std::string, std::string> {
public:
  bool leaf;
  ...
  explicit SchemaPoolNode_File(const bool _leaf, std::string _path, std::string _fileName) : leaf(_leaf),
  ...
  static std::shared_ptr<SchemaPoolNode_File> loadFromFile(const std::string &);
...
inline void SchemaPool_File::insert(std::string key) {
  std::ifstream file(source + "/root.dat", std::ios::binary);

  if (!file) {
    CreateDirectory(source.c_str(), nullptr);

    auto rootNode = std::make_shared<SchemaPoolNode_File>(true, source, "/root.dat");
    ...
    if (some)
      throw BTreeError("Схема с ключом = \"" + key + "\" уже существует!\nПроверьте ключ и повторите попытку\n");

    if (SchemaPoolNode_File::loadFromFile(rootFile)->keys.size() == 2 * T - 1) {
      auto newRootNode = std::make_shared<SchemaPoolNode_File>(false, source, "/root.dat");

      newRootNode->splitChild(0, rootFile);
      newRootNode->saveToFile(rootFile);
    }

    std::shared_ptr<SchemaPoolNode_File> node = SchemaPoolNode_File::loadFromFile(rootFile);
...
inline void SchemaPool_File::remove(const std::string &key) {
  if (!rootFile.empty() && SchemaPoolNode_File::loadFromFile(rootFile))
    SchemaPoolNode_File::loadFromFile(rootFile)->remove(key);

  if (SchemaPoolNode_File::loadFromFile(rootFile)->keys.empty())
    deleteDirectory(StringToWString(removeFileName(rootFile, "root.dat")));
}

//----------------------------------------------------------------------------------------------------------------------

inline std::shared_ptr<StringNodeForPool_File> SchemaPool_File::search(const std::string &key) {
  if (SchemaPoolNode_File::loadFromFile(source + "/root.dat"))
    return SchemaPoolNode_File::loadFromFile(source + "/root.dat")->search(key);
...
inline void SchemaPoolNode_File::insertNonFull(std::string &key) {
...
inline void SchemaPoolNode_File::splitChild(const int i, const std::string &yFile) {
  ...
  auto z = std::make_shared<SchemaPoolNode_File>(y->leaf, path, z_filename);
...
inline std::shared_ptr<StringNodeForPool_File> SchemaPoolNode_File::search(const std::string &key) {
  int idx = 0;
...
inline std::shared_ptr<CollectionSchema_File> SchemaPool_File::load(const std::string &key) {
  const std::shared_ptr<StringNodeForPool_File> &data = search(key);

  if (data)
    return std::make_shared<CollectionSchema_File>(data->data + '/' + data->key);
...
inline void SchemaPoolNode_File::remove(const std::string &key) {
  int idx = 0;
  auto _ = keys;
  ...
  if (idx < keys.size() && keys[idx]->key == key) {
    ...
    if (leaf)
      throw BTreeError("Схемы с ключом = \"" + key + "\" не существует существует!\nПроверьте ключ и повторите попытку\n");
...
inline void SchemaPoolNode_File::removeFromNonLeaf(const int &idx) {
...
inline void SchemaPoolNode_File::saveToFile(const std::string &filepath) const {
...
inline std::shared_ptr<SchemaPoolNode_File> SchemaPoolNode_File::loadFromFile(const std::string &path) {
  ...
  auto node = std::make_shared<SchemaPoolNode_File>(leaf, _path, _fileName);
...
inline void SchemaPoolNode_File::borrowFromNext(const int &idx) {
...
inline void SchemaPoolNode_File::borrowFromPrev(const int &idx) {
  auto child = loadFromFile(childrenFiles[idx]);
  auto sibling = loadFromFile(childrenFiles[idx - 1]);
...
inline void SchemaPoolNode_File::merge(const int &idx) {
  auto child = loadFromFile(childrenFiles[idx]);
  auto sibling = loadFromFile(childrenFiles[idx + 1]);
...
inline std::shared_ptr<StringNodeForPool_File> SchemaPoolNode_File::getPred(const int &idx) {
...
inline std::shared_ptr<StringNodeForPool_File> SchemaPoolNode_File::getSucc(const int &idx) {
...
inline void SchemaPoolNode_File::fill(const int &idx) {

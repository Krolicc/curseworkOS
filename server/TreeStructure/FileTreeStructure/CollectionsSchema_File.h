#ifndef COLLECTIONSCHEMA_FILE_H
#define COLLECTIONSCHEMA_FILE_H

#include <utility>

#include "../../ExtraFunctions.h"

#include "DataWrapper_File.h"
...
class CollectionSchema_File final : public BTree_File<std::string, std::string, DataWrapper_File> {
public:
  std::string rootFile;
  std::string source;

  explicit CollectionSchema_File(std::string _source) : source(std::move(_source)) {
  }
  ...
  std::shared_ptr<DataWrapper_File> load(const std::string &key, StringPool &strPool);
...
class CollectionSchemaNode_File final : public BTreeNode_File<std::string, std::string> {
public:
  bool leaf;
  ...
  explicit CollectionSchemaNode_File(const bool _leaf, std::string _path, std::string _fileName) : leaf(_leaf),
  ...
  static std::shared_ptr<CollectionSchemaNode_File> loadFromFile(const std::string &);
...
inline void CollectionSchema_File::insert(std::string key) {
  std::ifstream file(source + "/root.dat", std::ios::binary);

  if (!file) {
    CreateDirectory(source.c_str(), nullptr);

    auto rootNode = std::make_shared<CollectionSchemaNode_File>(true, source, "/root.dat");
    ...
    if (some)
      throw BTreeError("Коллекция с ключом = \"" + key + "\" уже существует!\nПроверьте ключ и повторите попытку\n");

    if (CollectionSchemaNode_File::loadFromFile(rootFile)->keys.size() == 2 * T - 1) {
      auto newRootNode = std::make_shared<CollectionSchemaNode_File>(false, source, "/root.dat");

      newRootNode->splitChild(0, rootFile);
      newRootNode->saveToFile(rootFile);
    }

    std::shared_ptr<CollectionSchemaNode_File> node = CollectionSchemaNode_File::loadFromFile(rootFile);
...
inline void CollectionSchema_File::remove(const std::string &key) {
  if (!rootFile.empty() && CollectionSchemaNode_File::loadFromFile(rootFile))
    CollectionSchemaNode_File::loadFromFile(rootFile)->remove(key);

  if (CollectionSchemaNode_File::loadFromFile(rootFile)->keys.empty())
    deleteDirectory(StringToWString(removeFileName(rootFile, "root.dat")));
}

//----------------------------------------------------------------------------------------------------------------------

inline std::shared_ptr<StringNodeForPool_File> CollectionSchema_File::search(const std::string &key) {
  if (CollectionSchemaNode_File::loadFromFile(source + "/root.dat"))
    return CollectionSchemaNode_File::loadFromFile(source + "/root.dat")->search(key);
...
inline void CollectionSchemaNode_File::insertNonFull(std::string &key) {
...
inline void CollectionSchemaNode_File::splitChild(const int i, const std::string &yFile) {
  ...
  auto z = std::make_shared<CollectionSchemaNode_File>(y->leaf, path, z_filename);
...
inline std::shared_ptr<StringNodeForPool_File> CollectionSchemaNode_File::search(const std::string &key) {
  int idx = 0;
...
inline std::shared_ptr<DataWrapper_File> CollectionSchema_File::load(const std::string &key, StringPool &strPool) {
  const std::shared_ptr<StringNodeForPool_File> &data = search(key);

  if (data)
    return std::make_shared<DataWrapper_File>(data->data, data->key, strPool);
...
inline void CollectionSchemaNode_File::remove(const std::string &key) {
  int idx = 0;
...
inline void CollectionSchemaNode_File::removeFromNonLeaf(const int &idx) {
...
inline void CollectionSchemaNode_File::saveToFile(const std::string &filepath) const {
...
inline std::shared_ptr<CollectionSchemaNode_File> CollectionSchemaNode_File::loadFromFile(const std::string &path) {
  ...
  auto node = std::make_shared<CollectionSchemaNode_File>(leaf, _path, _fileName);
...
inline void CollectionSchemaNode_File::borrowFromNext(const int &idx) {
...
inline void CollectionSchemaNode_File::borrowFromPrev(const int &idx) {
  auto child = loadFromFile(childrenFiles[idx]);
  auto sibling = loadFromFile(childrenFiles[idx - 1]);
...
inline void CollectionSchemaNode_File::merge(const int &idx) {
  auto child = loadFromFile(childrenFiles[idx]);
  auto sibling = loadFromFile(childrenFiles[idx + 1]);
...
inline std::shared_ptr<StringNodeForPool_File> CollectionSchemaNode_File::getPred(const int &idx) {
...
inline std::shared_ptr<StringNodeForPool_File> CollectionSchemaNode_File::getSucc(const int &idx) {
...
inline void CollectionSchemaNode_File::fill(const int &idx) {
